...
def initialize_tree(input_ids, model, past_key_values, logits_processor, embed_model, pixel_values, image_sizes, output_draft_attention_scores=False,  original_prompt_length=None, use_prune_head=None, head_ratio=None, ratio=None):
    ...
    last_image_position = (input_ids[0] == 32000).nonzero()[-1, 0].item()
    image_start = 5
    ...
    keep_indexs = torch.cat((torch.arange(image_start,device=device), top_image_attention_rank_index, torch.arange(image_end,input_ids.shape[1],device=device))).cpu()
    keep_indexs = keep_indexs.sort().values

    draft_input_ids = input_ids[:, keep_indexs] 
    ...
    if output_draft_attention_scores:
        draft_tokens, retrieve_indices, tree_mask, tree_position_ids, draft_attention_scores = model.ea_layer.topK_genrate(
            hidden_states, draft_input_ids, model.base_model.lm_head, logits_processor, input_embeds, output_draft_attention_scores=True, original_prompt_length=original_prompt_length, image_start=image_start, image_end=image_end, text_start=text_start, text_end=text_end, use_prune_head=use_prune_head, head_ratio=head_ratio
        )
        # target_score = []
        # for score in target_scores:
        #     target_score.append(score[:, image_start:text_end])
        target_score = target_scores[:, image_start:text_end]
        draft_score = draft_attention_scores
    ...
    return draft_input_ids, draft_tokens, retrieve_indices,tree_mask,tree_position_ids, orig, hidden_states, token, target_score, draft_score, image_start, image_end, text_start, text_end,
...
def update_inference_inputs(
    ...
    if output_draft_attention_scores:
        ...
        draft_attention_scores = None


    new_token += accept_length + 1

    return input_ids, draft_input_ids, draft_tokens, retrieve_indices,tree_mask,tree_position_ids, new_token, None, token, draft_attention_scores
